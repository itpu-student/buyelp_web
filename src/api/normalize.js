...
export function normalizeReview(api) {
  ...
  return {
    ...
    text: api.text || "",
    media: Array.isArray(api.media) ? api.media.filter(Boolean).map(staticUrl) : [],
    _mediaKeys: Array.isArray(api.media) ? api.media.filter(Boolean) : [],
    date: api.created_at,
    ...
    place: api.place ? {
      ...
      slug: api.place.slug,
      avgRating: Number(api.place.avg_ratings?.star) || 0,
